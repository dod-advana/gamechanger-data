...
function eda_files() {
    ...
    echo "---------------------------------------------------------------"
    "$PYTHON_CMD" -m  dataPipelines.gc_eda_pipeline.gc_eda_ocr_pipeline --staging-folder $LOCAL_GC_DIR --aws-s3-input-pdf-prefix $AWS_S3_INPUT_PDF_PREFIX --workers-ocr $WORKER_OCR --max-workers $MAX_WORKERS
}
echo "***************************** Start *****************************"
