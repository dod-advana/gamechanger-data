...
set -o pipefail
SECONDS=0
#EDA_JOB_TYPE=update_metadata_skip_new
#EDA_JOB_TYPE=normal
#EDA_JOB_TYPE=reindex
EDA_JOB_TYPE=reprocess
...
function eda_files() {
    ...
    echo "---------------------------------------------------------------"
    "$PYTHON_CMD" -m dataPipelines.gc_eda_pipeline.gc_eda_pipeline --staging-folder $LOCAL_GC_DIR --aws-s3-input-pdf-prefix $AWS_S3_INPUT_PDF_PREFIX --workers-ocr $WORKER_OCR --max-workers $MAX_WORKERS --eda-job-type $EDA_JOB_TYPE
}
echo "***************************** Start *****************************"
