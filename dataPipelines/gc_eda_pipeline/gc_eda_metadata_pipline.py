...
from dataPipelines.gc_eda_pipeline.database.database import audit_failed_record
from dataPipelines.gc_eda_pipeline.database.database import audit_success_record
from dataPipelines.gc_eda_pipeline.database.database import audit_fetch_all_records_for_base_path
...
def run(aws_s3_input_pdf_prefix: str, max_workers: int):
    ingestion(aws_s3_input_pdf_prefix=aws_s3_input_pdf_prefix, max_workers=max_workers)


def ingestion(aws_s3_input_pdf_prefix: str, max_workers: int):
    ...
    for input_loc in aws_s3_input_pdf_prefix.split(","):
        ...
        map_filename_audit_record = audit_fetch_all_records_for_base_path(input_loc)
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            results = [executor.submit(process_doc, filename, map_filename_audit_record[filename], data_conf_filter) for filename in map_filename_audit_record]
            count = 0
            none_type = type(None)
