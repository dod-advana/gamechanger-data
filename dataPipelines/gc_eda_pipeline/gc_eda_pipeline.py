...
def process_doc(file: str, staging_folder: Union[str, Path], data_conf_filter: dict, multiprocess: int,
    ...
    if re_index_only:
        ...
        if is_pdf_file:
            ex_file_local_path, ex_file_s3_path = extract_text(staging_folder=staging_folder, path=path,
                                                               pdf_file_local_path=pdf_file_local_path,
                                                               filename_without_ext=filename_without_ext,
                                                               aws_s3_json_prefix=aws_s3_json_prefix, audit_id=audit_id,
                                                               audit_rec=audit_rec, publish_audit=publish_audit)
            files_delete.append(pdf_file_local_path)
...
def generate_metadata_file(staging_folder: str, data_conf_filter: dict, file: str, filename: str,
                           ...
                           publish_audit: ConfiguredElasticsearchPublisher, skip_metadata: bool):
    ...
    is_md_successful, md_type, data = metadata_extraction(staging_folder, file, data_conf_filter, aws_s3_output_pdf_prefix, skip_metadata)
...
def ocr_process(staging_folder: str, file: str, multiprocess: int, aws_s3_output_pdf_prefix: str, audit_id: str,
                audit_rec: dict, publish_audit: ConfiguredElasticsearchPublisher):
...
def extract_text(staging_folder: str, pdf_file_local_path: str, path: str, filename_without_ext: str,
                 ...
                 publish_audit: ConfiguredElasticsearchPublisher):
    ...
    is_dp = os.path.exists(ex_file_local_path)
    doc_time_end = time.time()
    time_dp = doc_time_end - doc_time_start

    audit_rec.update({"json_path_s": ex_file_s3_path, "is_docparser_b": is_dp,
                      "docparser_time_f": round(time_dp, 4), "modified_date_dt": int(time.time())})
    audit_record_new(audit_id=audit_id, publisher=publish_audit, audit_record=audit_rec)

    return ex_file_local_path, ex_file_s3_path


def index(publish_es: ConfiguredElasticsearchPublisher, staging_folder: str, md_file_local_path: str,
    ...
    record_id = hashlib.sha256(ex_file_s3_path.encode())
    is_index = publish_es.index_json(index_output_file_path, record_id)
    index_end = time.time()
...
def create_index(index_name: str, alias: str):
    publisher = ConfiguredElasticsearchPublisher(index_name=index_name, ingest_dir="",  alias=alias)
...
def audit_record_new(audit_id: str, publisher: ConfiguredElasticsearchPublisher, audit_record: dict):
    publisher.insert_record(id=audit_id, json_record=audit_record)
...
def audit_complete(audit_id: str, publisher: ConfiguredElasticsearchPublisher, directory: str, number_of_files: int,
    ...
    publisher.insert_record(id=audit_id, json_record=ar)
