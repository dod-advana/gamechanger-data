...
@click.option("--loop-number", help="You should not need to change this value", required=False, type=int, default=50000)
@click.option(
    "--skip-metadata",
...
def run(
    ...
    for input_loc in aws_s3_input_pdf_prefix.split(","):
        ...
        eda_audit_publisher = create_index(
            index_name=data_conf_filter["eda"]["audit_index"], alias=data_conf_filter["eda"]["audit_index_alias"]
        )
        eda_publisher = create_index(
            index_name=data_conf_filter["eda"]["eda_index"], alias=data_conf_filter["eda"]["eda_index_alias"]
        )
                        ...
                        try:
                            if fut.result() is not None:
                                ...
                                if "already_processed" == status:
                                    ...
                                    print(
                                        f"Following file {fut.result().get('filename')} was processed, extra info: "
                                        f"{fut.result().get('info')}"
                                    )
                                    number_file_processed = number_file_processed + 1
                                elif "failed" == status:
                                    ...
                                    print(
                                        f"Following file {fut.result().get('filename')} was skipped, extra info: "
                                        f"{fut.result().get('info')}"
                                    )
        ...
        audit_complete(
            audit_id=audit_id + "_" + str(time.time()),
...
def process_doc(
    file: str,
    staging_folder: Union[str, Path],
    data_conf_filter: dict,
    multiprocess: int,
    aws_s3_output_pdf_prefix: str,
    aws_s3_json_prefix: str,
    ...
    audit_rec = {
        ...
        "is_metadata_suc_b": False,
        "is_ocr_b": False,
        "is_docparser_b": False,
        "is_index_b": False,
        "metadata_time_f": False,
        "ocr_time_f": 0.0,
        "docparser_time_f": 0.0,
        "index_time_f": 0.0,
        "modified_date_dt": 0,
    }
    ...
    if not is_process_already:
        ...
        process_type == EDAJobType.UPDATE_METADATA
        or process_type == EDAJobType.RE_INDEX
        or process_type == EDAJobType.UPDATE_METADATA_SKIP_NEW
    ) and is_process_already:
        audit_rec_old = publish_audit.get_by_id(audit_id)
        ...
        if not is_index_b:
            ...
            if check_if_legacy_audit_data is not None:
                ...
                if "json_path_s" not in audit_rec_old:
                    ...
                    audit_rec_old["json_path_s"] = (
                        path.replace("/pdf/", "/json/", 1) + "/" + gc_p_s_fn_without_ext + ".json"
                    )
    ...
    if re_index_only:
        ...
        index_output_file_path = index(
            staging_folder=staging_folder,
            publish_es=publish_es,
            path=path,
            filename_without_ext=filename_without_ext,
            md_file_local_path=md_file_local_path,
            ex_file_local_path=ex_file_local_path,
            ex_file_s3_path=ex_file_s3_path,
            audit_id=audit_id,
            audit_rec=audit_rec,
        ...
        md_file_s3_path, md_file_local_path = generate_metadata_file(
            staging_folder=staging_folder,
        ...
        is_pdf_file, pdf_file_local_path, pdf_file_s3_path = ocr_process(
            staging_folder=staging_folder,
            file=file,
        ...
        if is_pdf_file and os.path.exists(pdf_file_local_path):
            ex_file_local_path, ex_file_s3_path, is_extract_suc = extract_text(
                staging_folder=staging_folder,
                path=path,
...
def generate_metadata_file(
    ...
    is_md_successful, is_supplementary_file_missing, md_type, data = metadata_extraction(
        staging_folder, file, data_conf_filter, aws_s3_output_pdf_prefix, skip_metadata
    )
    ...
    audit_rec.update(
        {
            "filename_s": filename,
            "eda_path_s": file,
            "metadata_path_s": md_file_s3_path,
            "metadata_type_s": md_type,
            "is_metadata_suc_b": is_md_successful,
            "is_supplementary_file_missing": is_supplementary_file_missing,
            "metadata_time_f": round(time_md, 4),
...
def ocr_process(
    ...
    audit_rec.update(
        {
            "gc_path_s": pdf_file_s3_path,
            "is_ocr_b": is_ocr,
            "ocr_time_f": round(time_ocr, 4),
            "modified_date_dt": int(time.time()),
        }
    )
    audit_record_new(audit_id=audit_id, publisher=publish_audit, audit_record=audit_rec)
...
def extract_text(
    ...
    audit_rec.update(
        {
            "json_path_s": ex_file_s3_path,
            "is_docparser_b": is_extract_suc,
            "docparser_time_f": round(time_dp, 4),
            "modified_date_dt": int(time.time()),
        }
    )
    audit_record_new(audit_id=audit_id, publisher=publish_audit, audit_record=audit_rec)
...
def index(
    publish_es: EDSConfiguredElasticsearchPublisher,
    staging_folder: str,
    md_file_local_path: str,
    ex_file_local_path: str,
    path: str,
    filename_without_ext: str,
    ex_file_s3_path: str,
    audit_id: str,
    ...
    audit_rec.update(
        {"is_index_b": is_index, "index_time_f": round(time_index, 4), "modified_date_dt": int(time.time())}
    )
    audit_record_new(audit_id=audit_id, publisher=publish_audit, audit_record=audit_rec)
...
def pdf_ocr(file: str, staging_folder: str, multiprocess: int) -> Tuple[bool, bool]:
    try:
        path, filename = os.path.split(file)
        is_ocr = False
        is_pdf_file = False
        if filename != "" and Conf.s3_utils.object_exists(object_path=file):
            ...
            try:
                if (
                    is_pdf(str(saved_file))
                    and not is_ocr_pdf(str(saved_file))
                    and not is_encrypted_pdf(str(saved_file))
                ):
                    is_pdf_file = True
                    ocr = PDFOCR(
...
def docparser(
    metadata_file_path: str, saved_file: Union[str, Path], staging_folder: Union[str, Path], path: str
) -> bool:
    """
...
def audit_complete(
    audit_id: str,
    publisher: EDSConfiguredElasticsearchPublisher,
    directory: str,
    number_of_files: int,
    number_file_failed: int,
    modified_date: int,
    duration: int,
):
    ar = {
