...
def audit_file_exist(filename: str, base_path) -> bool:
    ...
    sql_audit_file_exist = """SELECT filename FROM public.gc_file_process_status WHERE filename = %s AND base_path <> %s; """
...
def audit_fetch_all_records_for_base_path(base_path: str):
    conn = psycopg2.connect(host=data_conf_filter['eda']['database']['hostname'],
                            port=data_conf_filter['eda']['database']['port'],
                            user=data_conf_filter['eda']['database']['user'],
                            password=data_conf_filter['eda']['database']['password'],
    ...
    with conn.cursor() as cursor:
        ...
        for row in rows:
            data_row = {
                ...
                "is_pds": row['is_pds'],
                "is_syn": row['is_syn'],
                "is_fpds_ng": row['is_fpds_ng'],
                "is_elasticsearch": row['is_elasticsearch'],
                "is_supplementary_file_missing": row['is_supplementary_file_missing']
            }
