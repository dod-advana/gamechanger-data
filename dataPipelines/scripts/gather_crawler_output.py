...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Gather metadata from a specific crawler.')
    parser.add_argument('--crawler', required=True, help='Crawler name')
    parser.add_argument('--input-dir', required=True, help='Input directory')
    parser.add_argument('--output', required=True, help='Output JSON file')

    args = parser.parse_args()
    
    # Access the arguments passed from the Bash script
    crawler = args.crawler
    dir = args.input_dir
    output_file = args.output

    lines = get_target_jsons(crawler, dir)
    result_txt = write_output_json(lines, output_file, os.getcwd())
    print(result_txt)
