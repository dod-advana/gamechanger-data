...
def upload_icon_data(
        icon_data_path="/home/gamechanger/de_test_scripts/icon_test_data/",
        excel_file_name="source_data_collection.xslx",
    ...
    related to each crawler from a specific excel spreadshet `excel_file_path`, and `icon_data_path` into:
    1. the s3 bucket folder `s3_prefix` (image)
    2. the postgres table `table_name` (metadata - s3 link, description)
    """
    excel_file_path = os.path.join(icon_data_path, excel_file_name)
    ...
    if missing_columns:
        ...
        for index, row in df.iterrows():
            ...
            if len(icon_fname) > 0:
                ...
                icon_fname_local_file_path = os.path.join(icon_data_path, icon_fname)
