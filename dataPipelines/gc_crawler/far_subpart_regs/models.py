...
class FarSubpartParser(Parser):
    ...
    def parse_docs_from_page(self, page_url: str, page_text: str) -> Iterable[Document]:
        ...
        # publication date of subparts will depend on the effective pub of FAR
        def get_date():
        ...
        # now scrape all subparts for FAR
        table = soup.find(
...
class FarSubpartCrawler(Crawler):
    """Crawler for the example web scraper"""
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
            **kwargs,
            pager=FarSubpartPager(
                starting_url=BASE_SOURCE_URL
            ),
            parser=FarSubpartParser()
        )
