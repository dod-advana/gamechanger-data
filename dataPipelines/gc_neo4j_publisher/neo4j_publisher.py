...
class Neo4jPublisher:
    ...
    def process_json_serial(self, filepath: str) -> str:
        id = -1
        try:
    ...
    def process_dir_serial(self, files: t.List[str], file_dir: str) -> None:
        print(f"process_dir_serial started", file=sys.stderr)
        try:
            if not files:
                return
            print(f"has files", file=sys.stderr)
            ...
            for filename in files:
                ...
                if filename.endswith('.json'):
                    ...
                    self.process_json_serial(filepath)
            
        except Exception as e:
            print(f"Error in process_dir serial {e}", file=sys.stderr)
            return
