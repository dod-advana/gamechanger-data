...
from dataScience.src.featurization.abbreviations_utils import get_agencies_dict, check_duplicates, get_agencies
...
class Table:
    ...
    def extract_section(self, input_dir):
        ...
        for file in sorted(os.listdir(input_dir)):
            ...
            temp_df["agencies"] = get_agencies(temp_df, doc_dups,
                                               self.duplicates, self.aliases)
            temp_df = self.get_entity_refs(temp_df)
            temp_df = self.get_agency(temp_df)
            logger.info("{:>25s} : {:>3,d}".format(self.doc_dict["filename"], len(temp_df)))  # noqa
    ...
    def num_let_parse(self, text, file, entity):
        ...
        while len(found)>1:
            ...
            elif vals[1] + "." in found[2] or chr(ord(vals[1]) + 1) + "." in \
                    found[2]:
                if levels[1] == 0:
                    level += 1
                    if level < 4:
    ...
    def get_entity_refs(self, df):
        ...
        # df["agencies"] = ""
        combined_cols = pd.DataFrame(df[df.columns[2:5]].apply(
            ...
            if isinstance(row["text"], str):
                ...
                if sum(1 for c in first if c.isupper()) > sum(
                    ...
                    if "shall" in row["text"].split(":", 1)[0]:
                        if row["text"].split("shall")[0].strip().split(" ")[-1] == "and":  # noqa
                            continue
    ...
    def get_agency(self, df):
        ...
        combined_cols = pd.DataFrame(df[df.columns[1:]].apply(
            lambda x: ','.join(x.dropna().astype(str)), axis=1),
                                     columns=['text'])
...
if __name__ == "__main__":
    ...
    table_obj = Table(args.input_dir,
                      args.output,
                      spacy_model_,
                      args.agencies_file,
                      args.glob)
