    ...
    try:
        with fitz.open(str(file_path)) as doc:
            ...
            if missing_text_page_count>0:
                return {'successful_ocr': False, "ocr_job_type": "redo-ocr", "bad_page_nums": bad_page_nums} #OCRJobType.REDO_OCR
            else:
                return {'successful_ocr': True, "ocr_job_type": "skip-text", "bad_page_nums": None} #OCRJobType.SKIP_TEXT
    except Exception as e:
        print(f"Unexpected error while trying to open {file_path}")
        print(e)
