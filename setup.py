...
setup(
    ...
    install_requires=list(filter(
        lambda p: not (p.lower().startswith('faiss-gpu')
                       and sys.platform != 'linux'),
        parse_requirements(REQUIREMENTS_PATH)
    )),
    extras_require={
        "devtools": parse_requirements(DEV_REQUIREMENTS_PATH),
        "ci": parse_requirements(CI_REQUIREMENTS_PATH),
        "covid-devtools": parse_requirements(CDVID19_REQUIREMENTS_PATH),
    },
    python_requires="==3.6.*",
    entry_points={
        'console_scripts': [
            'composectl=devtools.composectl.__main__:main',
            'configuration=configuration.__main__:main',
            'gci=dataPipelines.gc_ingest.__main__:cli',
            'uni=devtools.universal_test_harness.__main__:cli'
        ]
    }
)
