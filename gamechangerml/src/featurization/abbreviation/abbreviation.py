...
#     text, dic: t.Union[t.Dict[str, t.Any], str] = ABBREVIATIONS_FILE
# ):
#     """
...
def expand_abbreviations_no_context(
    text, dic: t.Union[t.Dict[str, t.Any], str] = ABBREVIATIONS_COUNTS_FILE
):
    """
    ...
    for key in words:
        if " " in key:
            if key in text:
                expansion.append(
                    max(lower[key.lower()], key=lambda k: lower[key.lower()][k])
                )
